...
Everything a Venezuelan business needs, condensed into one point. The page is a bright, sturdy sheet of paper: a warm off-white ground, deep navy ink that reads as space without ever going black, and one amber key that is lit at most once per section. Rules are solid and thick (2–3px), corners are barely softened (6px), there are no gradients and no glow. The sole authored moment is the ledger: the day's sales total counting up through a blackout, the panel going navy while the number keeps climbing, and the day closing «cuadrado» with a green check on every problem the system handled.
...
Motion is a single rehearsed sequence, not ambient decoration: the ledger plays once on mount (about eight seconds), the mark's rings breathe slowly where the mark appears, and everything stops under reduced motion. Content paints without JavaScript; reveal animations are gated on `scripting: enabled` and the site is a static export.
...
### Primary
- **Deep Navy** (`navy`): the ink of the world. Headlines, section titles, borders, the ledger's blackout ground, the singularity point, the `Cómo`/`Termómetro`/`Transparencia` section grounds, the highlighted price card, the footer, selection background, scrollbar thumb, form `accent-color`. 10.8:1 on paper.
...
The grid is a two-column story at md/lg: hero is `md:grid-cols-2` with headline and lead left and the ledger right (three grid children, so on phones the ledger sits between headline and lead); `Síntomas` and `Contacto` split roughly 1.1:1 / 1:1.2 at lg with gaps of 40–64px; pricing is 1 → 2 → 3 columns with 20px gutters. On phones everything stacks in one column: the ledger shows a single tape line under a one-sentence lead, so headline, panel, lead and the amber key fit in one phone screen. Phone sections are budgeted to about one screen each (2026-08-24): `py-8` ground, `Termómetro` as a snap-scroll row of tiles, `Precios` as a ladder of disclosure rows with the recommended rung open, `Equipo` as a list, `Calculadora` with a sticky total bar under the header, and the two optional diagnostic questions folded behind a disclosure.
...
One solid point (r 6.5) with two firm rings (r 17 at 3.5px, r 28 at 3px) in a 64-unit box, inline SVG in `currentColor`. At rest the rings sit at 80% and 45% opacity; with `breathe` they pulse between 35% and 100% over 3.2s, the outer ring offset by 0.6s. It appears in the `Cómo` section title and steps (paper, `mark` on), in the footer (paper, 20px), and in green in the submit modal as the success moment. `SectionTitle` defaults `mark` off; it is not a decorative bullet.

### Cierre de caja (signature)
The hero's one authored moment (2026-09-03; it replaced the drawn convergence). A 3px-navy bordered panel on paper: «Ventas de hoy» as a small Archivo label with an «ejemplo» tag, the figure in Archivo 900 tabular (`$` at 0.48em), and the day's log under a 2px `navy/20` rule. `CierreDeCaja` is a client component that server-renders its final state (all seven lines, the full total) and, on mount, replays the day once: 24 sample sales tick the figure up every 290ms (each one bumping it 3.5%), the log prints one event at a time (0 / 1.3 / 3.3 / 4.3 / 5.3 / 6.3 / 7.5s), each line growing in from zero height so the panel expands smoothly instead of reserving its final size, and the newest line is bold. Between «Se fue la electricidad» and «Volvió la electricidad» the panel is navy with paper text and the number keeps climbing. The figure starts at 1.3× (one line printed) and settles to 1× as the log fills, so the panel is never half empty. Events from the third on carry a green 3.5-stroke check. On phones the log collapses to one tape line showing the current event (five of the seven). Sample figures only, tagged «ejemplo», with a source line under the panel at md+ naming xinventory. Stops under reduced motion (final state).
